...
#define TB_OBJ m_dut->tb_ibex->vlSymsp->TOP__tb_ibex
...
volatile bool sigint_recevied = false;

static struct option long_options[] = {{"elf", required_argument, 0, 'f'},
...
static void sigint_handler(int s) { sigint_recevied = true; }
...
class bootstrap : public mem_api {
  public:
    ...
    uint8_t read(uint32_t addr) { return read_itcm(addr); }
    ...
    void dump()
    {
        ...
        for (int lo = lo_addr; lo < hi_addr; lo += 8) {
            printf("%02X %02X %02X %02X %02X %02X %02X %02X\n", read(lo + 0), read(lo + 1), read(lo + 2), read(lo + 3),
                   read(lo + 4), read(lo + 5), read(lo + 6), read(lo + 7));
        }
    }
};
...
int main(int argc, char **argv)
{
    ...
    const std::unique_ptr<Vtb_ibex> top{new Vtb_ibex{contextp.get(), "TOP"}};
    ...
    while (!contextp->gotFinish() && !(max_cycles != -1 && cycles >= max_cycles) && !sigint_recevied) {
        contextp->timeInc(CLK_PERIOD / 2);
        top->clk_i = !top->clk_i;
        top->eval();
        cycles += 1;
