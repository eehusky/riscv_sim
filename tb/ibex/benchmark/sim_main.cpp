...
public:
    ...
    void write_itcm(uint32_t addr, uint8_t data)
    {
        ...
        m_dut->tb_ibex->vlSymsp->TOP__tb_ibex.write_itcm(addr, data);
    }
    uint8_t read_itcm(uint32_t addr)
    {
        return m_dut->tb_ibex->vlSymsp->TOP__tb_ibex.read_itcm(addr);
    }
    void write_dtcm(uint32_t addr, uint8_t data)
    {
        addr = addr-DTCM_BASE;
        m_dut->tb_ibex->vlSymsp->TOP__tb_ibex.write_dtcm(addr, data);
    }
    uint8_t read_dtcm(uint32_t addr)
    {
        addr = addr-DTCM_BASE;
        return m_dut->tb_ibex->vlSymsp->TOP__tb_ibex.read_dtcm(addr);
    }
