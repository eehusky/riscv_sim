...
async def test_demux_by_region(dut,region_def=REGIONS[0], init_def=INITIATORS[1]):
    tb = TB(dut)
    initname, attr, index  = init_def
    obi = OBI(getattr(dut,attr)[index], tb, index)
...
async def test_demux_addrspace(dut):
    tb = TB(dut)
    obis = [
        OBI(getattr(dut,attr)[index], tb, index)
        for initname, attr, index
        in INITIATORS
    ]

    #obi = OBI(dut.xbar_initiators[1], tb, 0)
    await tb.cycle_reset()
    ...
    print("wait random")
    for _ in range(5000):
        if all([obi.rsp_queue.empty() and obi.req_queue.empty() and obi.pend_queue.empty() for obi in obis]):
            #if obi.rsp_queue.empty() and obi.req_queue.empty() and obi.pend_queue.empty():
            break
        await tb.clkcycle(100)
    else:
...
async def test_demux_oob(dut):
    tb = TB(dut)
    obi = OBI(dut.xbar_initiators[0], tb, 0)
    await tb.cycle_reset()
